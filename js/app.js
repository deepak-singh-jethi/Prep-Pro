                ...
                <div onclick="app.openTaskModal('${dateStr}')"
                    ...
                    <div class="flex flex-col gap-1">
                        <div class="flex gap-1 mb-1 items-center">
                            ${totalTasks > 0 ? tasksForDay.slice(0, 4).map(t =>
                                `<div class="w-1.5 h-1.5 rounded-full ${t.status === 'done' ? 'bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.6)]' : 'bg-gray-400'}"></div>`
                            ).join('') : '<div class="w-1 h-1 rounded-full bg-slate-800"></div>'}
                        </div>
